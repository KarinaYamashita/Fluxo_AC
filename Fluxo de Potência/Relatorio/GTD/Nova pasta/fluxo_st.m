...
function calc_fluxo
    hl = '\\\hline';
    fprintf('De  Para     Pkm       Qkm       Pmk       Qmk \n')
    for i = 1:length(a)
        k = a(i);
        m = b(i);
        Pkm = +V(k)^2 * G(k,m) - V(k)*V(m)*G(k,m)*cos(T(k)-T(m)) - V(k)*V(m)*B(k,m)*sin(T(k)-T(m));
        Qkm = -V(k)^2 * B(k,m) + V(k)*V(m)*B(k,m)*cos(T(k)-T(m)) - V(k)*V(m)*G(k,m)*sin(T(k)-T(m));
        Pmk = +V(m)^2 * G(k,m) - V(k)*V(m)*G(k,m)*cos(T(k)-T(m)) + V(k)*V(m)*B(k,m)*sin(T(k)-T(m));
        Qmk = -V(m)^2 * B(k,m) + V(k)*V(m)*B(k,m)*cos(T(k)-T(m)) + V(k)*V(m)*G(k,m)*sin(T(k)-T(m));
        Pt = Pt + Pkm + Pmk;
        Qt = Qt + Qkm + Qmk;
